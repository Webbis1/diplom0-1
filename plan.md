...
### Core (Ядро)
- **Logic:** [Graph](src/application/logic/graph.py) • [Node](src/application/logic/node.py) • [Edge](src/application/logic/edge.py) • [Potential](src/application/logic/potential.py)
...
## Этап 1: Завершение математического ядра ([Graph](src/application/logic/graph.py) + тесты)
- [ ] Добавить метод `get_optimal_route(coin, exchange)` в [Graph](src/application/logic/graph.py) с использованием [AsyncRLock](src/core/utils/async_rlock.py)
- [ ] Добавить метод `get_best_deal(coin, exchange)` в [Graph](src/application/logic/graph.py) (обертка для [IAnalyst](src/application/interfaces/analyst.py))
- [ ] Написать unit-тест для [Potential](src/application/logic/potential.py): проверка лексикографического сравнения
- [ ] Написать unit-тест для [Edge](src/application/logic/edge.py): проверка формул пересчета с фиксированной комиссией
- [ ] Написать unit-тест для [Node](src/application/logic/node.py): проверка жадного выбора лучшего ребра
- [ ] Написать интеграционный тест для [Graph](src/application/logic/graph.py): создать 3 вершины, 2 ребра, запустить волну, проверить сходимость через `wait_completion()`
- [ ] Запустить тесты: `pytest tests/test_core_logic.py`

## Этап 2: Создание [Analyst](src/infrastructure/base/analyst.py) (обертка над [Graph](src/application/logic/graph.py))
...
- [ ] Добавить поле `self._graph` типа [Graph](src/application/logic/graph.py)
- [ ] Реализовать метод `get_best_deal(coin, exchange)` через вызов `Graph.get_best_deal()`
- [ ] Написать тест для `Analyst`: мокируем [Graph](src/application/logic/graph.py), проверяем delegation
...
- [ ] Обновить [app.py](src/app.py): добавить создание [Graph](src/application/logic/graph.py)
- [ ] Обновить [app.py](src/app.py): добавить вызов `Graph.start()`
- [ ] Обновить [app.py](src/app.py): добавить создание [Analyst](src/infrastructure/base/analyst.py) с инъекцией [Graph](src/application/logic/graph.py)
...
- [ ] Добавить поле `route` типа `List[Node]` (где [Node](src/application/logic/node.py)) в [Deal](src/core/entities/deal.py)
...
- [ ] Обновить метод `get_best_deal()` в [Graph](src/application/logic/graph.py) для возврата заполненного [Deal](src/core/entities/deal.py)
...
- [ ] Обновить [GraphVisualizer](src/visualizer/graph_visualizer.py): добавить метод `update_graph(graph)`, где `graph` — это [Graph](src/application/logic/graph.py)
- [ ] Добавить генерацию `graph.html` с PyVis
- [ ] Добавить раскраску узлов: красный если `potential.a > 1.01` (где [Potential](src/application/logic/potential.py))
- [ ] Добавить раскраску ребер: зеленый если `multiplier > 1.001`
- [ ] Добавить тултипы с `Potential(a, b)` (см. [Potential](src/application/logic/potential.py))
...
- [ ] Проверить, что [Graph](src/application/logic/graph.py) корректно останавливается при Ctrl+C
